...
/**
 * Generate C# enum (FieldTypeKind) + string converters for FieldType.
 * The enum gives callers a closed, compile-time-checked set of field types (the string
 * constants in FieldType cannot serve as a parameter type). The converters map each enum
 * member to its FieldType constant, so a removed/renamed field type breaks compilation here.
 */
function generateCSharpFieldTypeEnum(fieldTypes) {
  const header = `// <auto-generated />
// This file is auto-generated from core/models/src/vault/Item.ts
// Do not edit this file directly. Run 'npm run generate:models' to regenerate.
...
public static class FieldTypeKindExtensions
{
    ...
    {
${fieldTypes.map(t => `        FieldTypeKind.${t} => FieldType.${t},`).join('\n')}
        _ => FieldType.${fieldTypes[0]},
    };

    /// <summary>
    /// Converts a canonical <see cref="FieldType"/> string to a <see cref="FieldTypeKind"/>.
    /// Unknown or null values fall back to <see cref="FieldTypeKind.${fieldTypes[0]}"/>.
    /// </summary>
    ...
    {
${fieldTypes.map(t => `        FieldType.${t} => FieldTypeKind.${t},`).join('\n')}
        _ => FieldTypeKind.${fieldTypes[0]},
    };
}
`;
...
function main() {
  ...
  // Generate C# FieldTypeKind enum (+ string converters)
  ensureDir(CS_FIELD_TYPE_ENUM_OUTPUT);
  const csFieldTypeEnumContent = generateCSharpFieldTypeEnum(fieldTypes);
  fs.writeFileSync(CS_FIELD_TYPE_ENUM_OUTPUT, csFieldTypeEnumContent, 'utf8');
  console.log(`Generated: ${CS_FIELD_TYPE_ENUM_OUTPUT}`);
